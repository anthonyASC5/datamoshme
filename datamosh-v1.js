...
const EXPORT_FILENAME = "datamosh-homepage.webm";
...
function cleanupRecorder() {
  ...
  exportButton.querySelector("span").textContent = "Record WebM";
}
...
async function startPreview() {
  ...
  setStatus(state.exporting ? "Recording WebM..." : "Preview running.");
...
async function exportPreview() {
  ...
  if (!mimeType) {
    setStatus("This browser cannot record WebM from the canvas.");
    return;
  }
  ...
  exportButton.querySelector("span").textContent = "Recording...";
  ...
  state.recorder.onstop = () => {
    ...
    cleanupRecorder();
    stopPlayback({ keepFrame: true, keepRecorder: true });
    setStatus(blob.size > 0 ? `Export complete. Downloaded ${EXPORT_FILENAME}.` : "Export failed.");
  };

  state.recorder.start();
  await startPreview();
  if (!state.previewRunning && state.recorder && state.recorder.state !== "inactive") {
    state.recorder.stop();
  }
}
